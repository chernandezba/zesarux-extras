...
; #                                                                       #
; # El listado está preparado para ser ensamblado con TASM, puede ser     #
...
;	1)	Al principio del fuente encontrarás un pseudo-nemónico
;		.TITLE "xxxxx" que no tiene utilidad alguna pero queda
;		bonito si imprimes por impresora HILOWROM.LST luego
;		de ensamblar. Con ciertos reparos podría ser equivalente
;		a "*H" del GENS.

;	2)	Al final del fuente encontrarás un pseudo-nemónico ".END"
...
;		no es obligatorio, pero TASM lo acepta y hace más legible
;		el listado.

;	4)	En el GENS se usa la notación #NN para indicar un número
;		en base 16, aquí se usa la notación $NN teniendo el mismo
;		significado.

; =============================================================================

; Equivalencias GENS -> TASM
#DEFINE		DEFB	.DB
#DEFINE		DEFW	.DW
#DEFINE		DEFS	.BLOCK
#DEFINE		DEFM	.TEXT
...
; Definición del nemónico "LD A,Ix"
...
START:		DI			;RST 00H el ejecutarse el RET en
		...
		DEFS	$01		;la ROM normal provocando un RESET.
					...
					;como espacio libre rellanados con
					;ceros.

RST08:		DEFS	$08		;RST 08H no hace nada, en principio
					;por contener 0's ejecuta un RST 10H
					
RST10:		RST	18H		;RST 10H al usar el RST 18H con un
		DEFW	RST10		;DW $0010, tiene la misma función
...
RST20:		LD	B,$FF		;RST 20H imprime un mensaje cuyos
		...
		DEFS	$03		;dirección DE, los códigos son impresos
					;hasta que B=0 o (DE)=0

RST28:		JP	PR_ERRMSG	;RST 28H imprime los mensajes
		DEFS	$05		;de error del HiLow, cuyo nro. viene
					;indicado en A.
...
MSCAT2:		DEFM	"\\BAS \\NUM \\CHR \\COD \\NMI  K LIBRES\000"
MSVER:		DEFB	"    HILOW  VER 1.3E",0
...
PROTECCION:	JR	NZ,L0EE0	; no?==>vamos a copiar
		...
		CALL	W_SOUND_SPC	; sonido de aviso
					...
					; pero entonces....
		CALL	TEST_HIW	; !!!aquí comprueba algo interesante!!!
...
SIN_USO1:	DEFS	$B2		;178 bytes libres
...
SIN_USO2:	DEFS	$14		;20 bytes libres
...
;			A PARTIR DE ESTA DIRECCION ESTAN LAS RUTINAS DEPENDIENTES DEL HARDWARE DEL DATA DRIVE
;			LAS REFERENCIAS A LAS ETIQUETAS QUE ESTAN A PARTIR DE ESTA DIRECCION DEBERIAN ESTAR TAMBIEN A PARTIR DE ESTA DIRECCION

;			EL UNICO ACCESO A RUTINAS QUE ESTAN A PARTIR DE ESTA DIRECCION DESDE DIRECCIONES ANTERIORES DEBE HACERSE A TRAVES DE LA TABLA DE SALTOS
;            QUE EMPIEZA EN LA ETIQUETA EX_RDSECT
...
; byte / sector / registro_de_estado / o_que_se_yó! :) :)
; desde el grabador.
;
...
MSGCPR:		DEFB	$16,$05,$04,$10,$09,$11,$08
		...
		DEFB	$16,$15,$01
		DEFB	"TEL. 91 47 73         VER 1.3E"


SIN_USO3:	DEFS	$98		; 152 bytes libres
...
SIN_USO4:	DEFS	$20		; 32 bytes libres
...
SIN_USO5:	DEFS	$76		;118 bytes libres
LAST_BYTE:	EQU	$
