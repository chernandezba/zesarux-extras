...
NMI:		LD	(NMI_STACK),SP	;RUTINA DE SERVICIO DE LA NMI
		LD	SP,START_RAM	;ÄÄ¿
		PUSH	AF		;  ÃÄ Obs‚rvese c¢mo se guardan
		...
		RETN			;ÄÄÙ ³
					;    ³
CONTINT:	AND	A		;ÄÄÄÄÁÄ Notar que aqu¡ contin£a la NMI
...
L06FF:		CALL	L1F46		;Esta parte aparentemente salva
		...
		RST	28H		;(IX+$0B) ÄÂÄ LONGITUD
		CALL	W_SOUND		;(IX+$0C) ÄÙ
		CALL	L0768		;(IX+$0D) ÄÂÄ COMIENZO
		LD	A,$00		;(IX+$0E) ÄÙ
		RST	28H		;(IX+$00) ÄÄÄ FLAG (*)
		LD	A,$01		;(IX+$01) ÄÄÄ NOMBRE (10 bytes)
		CCF			;
...
PARSE_SAVE:	POP	HL
		...
		CALL	W_SOUND
		LD	HL,$1392	; ÄÄ¿
		RST	18H             ;   Ã Aqu¡ decide si es ROM SPECTRUM
...
L088A:		LD	A,$0F		;Mensaje: "PONGA CASSETTE A FORMATEAR"
		...
		IN	A,(HLWPORT)	;				     ³
		BIT	2,A		;				     ³
		JR	Z,L088A		;repite si ni hay cassette puesto ÄÄÄÙ
		LD	A,$10
